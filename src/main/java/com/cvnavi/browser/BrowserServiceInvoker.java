...
public class BrowserServiceInvoker  extends AbstractDailyTask implements AutoCloseable {
	...
	static {
		ServletContextCleaner.registeCloseable(new BrowserServiceInvoker());
	}
	...
	public static String getPage(String url, HttpHost proxy) {
		try {
			String cmd = "cmd=" + BrowserService.CMD_GET_PAGE + "&url=" + URLEncoder.encode(url, "UTF-8");
			if (proxy != null) {
				cmd += "&proxy=" + URLEncoder.encode(proxy.getHostName() + ":" + proxy.getPort(), "UTF-8");
			}
			return sendCmd(cmd);
	...
	public static synchronized void stopBrowserService() {
		...
		try {
			socket = new Socket("127.0.0.1", BrowserService.port);
			socket.getOutputStream().write(("cmd=" + BrowserService.CMD_EXIT).getBytes());
		} catch (IOException e) {
		} finally {
			try {
				if (socket != null) {
